...
# umbre_s13_SMD_movie()
# {
# 	ScanTRAJ_SMD; variables_for_SMD_Movie; analyser11;
...
MovChoic
		...
		if [[ "$moviechoic" == "a" ]]; then analyser11
		elif [[ "$moviechoic" == "b" ]]; then analyser11update
		fi
	elif [[ "$stage" == 13 ]] && [[ ! -d "$movieDIRECORY" ]]; then analyser11
	fi
}
